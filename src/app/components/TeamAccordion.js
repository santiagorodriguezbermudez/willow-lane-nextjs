...
const TeamMemberAccordion = ({ name, title, description, slug, isOpen, onClick }) => {
  return (
    <div className="border-b border-gray-light/50 text-gray-light relative">
      <button
        className="w-full py-4 flex items-center justify-between hover:bg-gray-light/10 transition-colors"
        onClick={onClick}
      >
        <div className="flex-1 flex items-center justify-start">
          <span className="font-haboro text-xl md:text-[24.7px] text-gray-light uppercase text-left border-r border-gray-light pr-8">{name}</span>
          <span className="font-thin text-lg md:text-[27.3px] text-gray-light text-right md:text-left pl-8">{title}</span>
        </div>
        <ChevronDownIcon 
          className={`w-8 h-8 md:w-10 md:h-10 min-w-8 min-h-8 md:min-w-10 md:min-h-10 transform transition-transform duration-300 ease-in-out text-gray-light flex-shrink-0 ml-4 [stroke-width:1] ${isOpen ? 'rotate-180' : ''}`}
        />
      </button>
      ...
      <div className={`active:ring-0 active:outline-none active:ring-offset-0 active:border-0 focus:outline-none focus:ring-0 focus:ring-offset-0 focus:border-0 transition-all duration-300 ease-in-out bg-white ${isOpen ? 'absolute top-100 left-1/2 w-50 z-[1000] right-[18px] h-auto top-[60px]' : 'hidden'}`}>
        <div className="px-4 md:px-6 py-4">
          <div>
            <h6 className="text-primary text-2xl md:text-[24.7px] font-haboro uppercase mb-1 font-thin">{name}</h6>
            <p className="text-gray-dark text-base md:text-[20.8px] italic font-source-sans font-thin mb-1">{title}</p>
            <p className="h-1 border-b border-primary w-1/2 mb-4"></p>
            <p className="text-gray-dark font-thin mb-8 font-source-sans text-base md:text-[20.8px]">{description}</p>
            ...
            <div className="flex justify-end">
              ...
              >
                <span className="text-gray-dark font-thin group-hover:text-primary transition-colors font-source-sans text-sm md:text-[23.4px]">
                  View Full Profile
                </span>
